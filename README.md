...
You are my product engineer inside ChatGPT Work. Build and publish a complete, private, mobile-friendly personal financial dashboard named **FINTRACK**. This is a generic personal-finance product and is not associated with any business.
...
7. Create or reuse one dedicated Google Drive folder exactly once. Use the exact folder name `FinTrack Financial Inbox`. If a folder with that exact name already exists in my Drive, reuse it instead of creating another one.
...
2. Search my Drive for a folder whose name is exactly `FinTrack Financial Inbox`.
...
2. Delete every object belonging to this FINTRACK Site from R2.
...
- **FINTRACK insight:** show useful, factual insights only, such as a count of transactions in `Needs review`. Do not generate fake savings advice from absent data.
...
Provide **Erase all FINTRACK data**. The confirmation modal must:
...
5. call `DELETE /api/state` with the server confirmation `DELETE ALL FINTRACK DATA`
...
After the Site and folder exist, create one automation titled **Sync FinTrack Inbox**.
...
You maintain the private FINTRACK financial Site at `<EXACT_SITE_URL>` using the dedicated Google Drive folder `<EXACT_DRIVE_FOLDER_NAME>` with ID `<EXACT_DRIVE_FOLDER_ID>`.
